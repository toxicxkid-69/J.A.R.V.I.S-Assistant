...
# =============================================================================
# THREAD-SAFE QUEUE-BASED TTS MANAGER (Fixes Windows SAPI5 COM Deadlocks)
# =============================================================================
class TTSManager:
    def __init__(self):
        self.q = queue.Queue()
...
def speak(audio: str) -> None:
    """Speaks out the audio message and prints to console and GUI feed."""
    print(f"\n[Jarvis]: {audio}")
    if gui_callback:
        try:
...
def execute_command(query: str) -> bool:
    ...
    # 2. Notes Management (Must precede generic checks, but exclude Notepad app)
    ...
    # 18. Explicit Searches: "search [query]" or "search [query] on google"
    if cleaned_q.startswith("search for ") or cleaned_q.startswith("search ") or "search google" in cleaned_q:
        term = re.sub(r'^(search\s+google\s+for|search\s+for|search\s+google|search)\s*', '', cleaned_q).strip()
    ...
    # 19. Universal Intelligent Fallback (Answers questions or searches any query cleanly)
    speak(f"Searching {raw_q}{b_label}")
    url = f"https://www.google.com/search?q={urllib.parse.quote(raw_q)}"
    open_in_browser(url, target_browser)
    return True

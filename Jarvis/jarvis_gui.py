...
class JarvisStarkHUD:
    def __init__(self, root: tk.Tk):
        ...
        self.wake_thread = threading.Thread(target=self._wake_word_daemon, daemon=True)
    ...
    def _build_ui(self):
        ...
        power_actions = [
            ("🔒 LOCK", lambda: self._send_command("lock pc")),
            ("💤 SLEEP", lambda: self._send_command("sleep pc")),
            ("⚡ RESTART", lambda: self._send_command("restart system")),
            ("❌ SHUTDOWN", lambda: self._send_command("shutdown system"))
        ]
    ...
    def _wake_word_daemon(self):
        """Passively listens in the background for the wake word 'Jarvis'."""
        time.sleep(2.5)  # Allow initial startup greeting to finish
        while self.wake_word_active:
            if self.is_listening or self.is_speaking:
                time.sleep(0.4)
                continue
            try:
                detected, remainder = jarvis.listen_for_wake_word(device_index=self.selected_device_index)
                if detected and not self.is_listening and not self.is_speaking:
                    if remainder:
                        # User spoke command directly with wake word e.g. "Jarvis what is 2 plus 2"
                        self._log("User", f"Jarvis, {remainder}")
                        self._process_command_worker(remainder)
                    else:
                        # User said "Jarvis" to activate voice listening!
                        self._log("Jarvis", "Yes, sir? I am listening.")
                        jarvis.speak("Yes, sir? I am listening.")
